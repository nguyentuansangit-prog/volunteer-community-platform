...
| 8 | Thông báo thay đổi trạng thái | Thông báo qua hệ thống hoặc email khi trạng thái đơn đăng ký/chiến dịch thay đổi. |

---

## 4. User Stories & Acceptance Criteria (AC)

**US-01: Quản lý tài khoản & Hồ sơ**
* **User Story:** As a User (Guest), I want to register and log in to the platform with a specific role, so that I can access features tailored to my permissions.
* **Acceptance Criteria:**
  * AC1.1: Cho phép đăng ký với vai trò Tình nguyện viên hoặc Đơn vị tổ chức.
  * AC1.2: Xác thực email hợp lệ và mật khẩu bảo mật tối thiểu 8 ký tự.
  * AC1.3: Cho phép người dùng/tổ chức cập nhật thông tin hồ sơ sau khi đăng nhập.

**US-02: Tìm kiếm & Xem hoạt động công khai**
...
* **Acceptance Criteria:**
  * AC2.1: Cho phép tìm kiếm theo từ khóa tiêu đề hoặc địa điểm.
  * AC2.2: Lọc danh sách theo trạng thái (Đang mở đăng ký, Sắp diễn ra, Đã kết thúc).
  * AC2.3: Hiển thị chi tiết thông tin: Mô tả, Số lượng cần tuyển, Hạn đăng ký, Địa điểm.

**US-03: Đăng ký & Hủy đăng ký tham gia**
* **User Story:** As a Volunteer, I want to apply for or cancel my application for a volunteer event, so that I can manage my participation schedule.
* **Acceptance Criteria:**
  * AC3.1: Chỉ tài khoản Volunteer đã đăng nhập mới thấy nút "Đăng ký tham gia".
  * AC3.2: Sau khi gửi đơn, trạng thái ban đầu mặc định là `PENDING`.
  * AC3.3: Hệ thống chặn đăng ký khi chiến dịch đã đủ số lượng tuyển hoặc đã quá hạn.
  * AC3.4: Cho phép Tình nguyện viên bấm "Hủy đăng ký" khi đơn còn ở trạng thái `PENDING`.

**US-04: Tạo, duyệt và công khai hoạt động**
* **User Story:** As an Organization, I want to create a new volunteer campaign, so that I can recruit volunteers.
* **Acceptance Criteria:**
  * AC4.1: Biểu mẫu bắt buộc điền: Tiêu đề, Nội dung, Số lượng TNV, Ngày bắt đầu/kết thúc, Hạn đăng ký.
  * AC4.2: Hoạt động sau khi tạo sẽ ở trạng thái `PENDING_APPROVAL` (Chờ Admin duyệt).
  * AC4.3: Khi Admin duyệt, hoạt động chuyển sang `PUBLISHED` và hiển thị công khai trên ứng dụng.

**US-05: Duyệt hoặc từ chối đăng ký**
...
* **Acceptance Criteria:**
  * AC5.1: Hiển thị danh sách các TNV đang chờ duyệt (`PENDING`).
  * AC5.2: Cho phép bấm "Chấp nhận" (`APPROVED`) hoặc "Từ chối" (`REJECTED`).
  * AC5.3: Tự động gửi thông báo đến TNV ngay khi trạng thái đơn thay đổi.

**US-06: Điểm danh & Ghi nhận kết quả**
...
* **Acceptance Criteria:**
  * AC6.1: Hiển thị danh sách TNV đã duyệt (`APPROVED`) để ban tổ chức điểm danh.
  * AC6.2: Đánh dấu "Có mặt" (`ATTENDED`) hoặc "Vắng mặt" (`ABSENT`).
  * AC6.3: Cập nhật số giờ tình nguyện tích lũy vào hồ sơ cá nhân của TNV sau khi xác nhận.

**US-07: Dashboard Thống kê**
* **User Story:** As an Organization/Admin, I want to view platform statistics, so that I can track campaign progress and volunteer participation.
* **Acceptance Criteria:**
  * AC7.1: Hiển thị tổng số chiến dịch đã tạo/hoàn thành.
  * AC7.2: Thống kê tổng số lượt đăng ký, số TNV đã tham gia và tổng số giờ đóng góp.

**US-08: Thông báo thay đổi trạng thái**
* **User Story:** As a User, I want to receive notifications when there are status changes, so that I stay updated promptly.
* **Acceptance Criteria:**
